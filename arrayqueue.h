...
#define DEFINE_STRUCT_QUEUE(struct_name, node_type) \
struct struct_name {                                \
    node_type *nodes;                               \
    size_t     size;                                \
    size_t     used;                                \
...
#define AQUEUE_NODES_FREE(q) ( (q)->size - (q)->used )
...
#define AQUEUE_FIN_GET(q)    ( (q)->rear  = ( (q)->rear  + 1 ) % (q)->size, (q)->used-- )
#define AQUEUE_FIN_PUT(q)    ( (q)->front = ( (q)->front + 1 ) % (q)->size, (q)->used++ )
